#!/usr/bin/env python
"""
MetaMath Instruction-Following Training Script
===============================================
Supports: lora, pissa, dora, jelly, bitfit, full_finetune
Dataset: pissa-dataset (MetaMath sub-task)
Ref: PiSSA/train.py
"""
...
from utils.test_acc import evaluate_results, print_acc_results
...
class DataCollatorForSupervisedDataset:
    """Collate examples for supervised fine-tuning (ref: PiSSA/train.py)."""
...
def main(args):
    ...
    # Dataset loading (ref: PiSSA/train.py)
    ...
    # ── Test accuracy evaluation (gen + test_acc) ──────────────────────────
    test_acc_results = {}
    if args.eval_test_acc:
        print("\n[TEST ACC] Loading test split for accuracy evaluation...")
        test_raw = load_dataset(
            args.data_path, data_dir="metamath", split="test"
        )

        # Filter by type if --eval_test_types is specified
        eval_types = [t.strip() for t in args.eval_test_types.split(",") if t.strip()]
        if eval_types:
            test_raw = test_raw.filter(lambda x: x["type"] in eval_types)
            print(f"[TEST ACC] Filtered to types={eval_types} → {len(test_raw)} samples")
        ...
        # Optionally save JSONL for later inspection / CLI test_acc.py
        if args.save_test_responses:
            resp_file = os.path.join(result_dir, f"{RESULT_PREFIX}_responses_{at}_r{args.r}_s{args.seed}.jsonl")
            save_responses_jsonl(responses, resp_file)

        test_acc_results = evaluate_results(responses)
        print_acc_results(test_acc_results)

        for key, val in test_acc_results.items():
            if key != "counts" and val is not None:
                result_data[f"test_{key}_acc"] = val

        if not args.no_wandb:
            import wandb
            if wandb.run is not None:
                for key, val in test_acc_results.items():
                    if key == "counts" or val is None:
                        continue
                    if key == "overall":
                        wandb.run.summary["eval/final_test_acc"] = val
                    else:
                        wandb.run.summary[f"eval/final_test_{key}_acc"] = val
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MetaMath Instruction-Following Training")
    ...
    parser.add_argument("--eval_test_acc", action="store_true",
                        help="Run generation-based test accuracy after training")
    parser.add_argument("--eval_test_types", type=str, default="math,gsm8k",
                        help="Comma-separated test types to evaluate (e.g. 'math' or 'math,gsm8k'). Empty=all")
    parser.add_argument("--max_test_samples", type=int, default=0,
                        help="Max test samples for eval_test_acc (0=all)")
    parser.add_argument("--test_max_new_tokens", type=int, default=256,
                        help="Max new tokens to generate during test_acc eval")
    parser.add_argument("--save_test_responses", action="store_true",
                        help="Save generated responses to JSONL file")
