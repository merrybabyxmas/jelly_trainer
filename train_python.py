#!/usr/bin/env python
"""
Python Code Generation Training Script
=======================================
Supports: lora, pissa, dora, jelly, bitfit, full_finetune
Dataset: pissa-dataset (python sub-task)
Evaluation: eval_loss (proxy) + evalplus pass@1 for HumanEval/MBPP (--eval_test_acc)
Ref: PiSSA/train.py
"""
...
from utils.gen_hf import generate_responses, save_responses_jsonl
from utils.code_process import process_code_completions, write_evalplus_jsonl
...
def main(args):
    ...
    if args.eval_test_acc:
        import re, subprocess, tempfile
        print("\n[PYTHON EVAL] Loading test split (HumanEval + MBPP)...")
        test_raw = load_dataset(args.data_path, data_dir="python", split="test")
        # NOTE: evalplus requires completions for ALL tasks in the dataset;
        # max_test_samples is not applied here (would cause evalplus assertion error).
        n_he = sum(1 for x in test_raw if x["type"] == "humaneval")
        n_mbpp = sum(1 for x in test_raw if x["type"] == "mbpp")
        print(f"[PYTHON EVAL] {len(test_raw)} test samples (humaneval:{n_he}, mbpp:{n_mbpp})")
        ...
        processed = process_code_completions(responses)

        with tempfile.TemporaryDirectory() as tmpdir:
            eval_results_code = {}
            for dataset_name, entries in processed.items():
                if not entries:
                    print(f"[PYTHON EVAL] No {dataset_name} samples, skipping.")
                    continue

                samples_file = os.path.join(tmpdir, f"{dataset_name}.jsonl")
                write_evalplus_jsonl(entries, samples_file)

                if args.save_test_responses:
                    save_path = os.path.join(
                        result_dir,
                        f"{RESULT_PREFIX}_responses_{at}_r{args.r}_s{args.seed}_{dataset_name}.jsonl"
                    )
                    write_evalplus_jsonl(entries, save_path)

                # Run evalplus evaluate
                ret = subprocess.run(
                    ["python", "-m", "evalplus.evaluate",
                     "--dataset", dataset_name, "--samples", samples_file],
                    capture_output=True, text=True
                )
                stdout = ret.stdout + ret.stderr
                match = re.search(r"pass@1\s*:\s*([\d.]+)", stdout)
                if match:
                    pass1 = float(match.group(1))
                    eval_results_code[dataset_name] = pass1
                    print(f"[PYTHON EVAL] {dataset_name} pass@1: {pass1:.4f}")
                else:
                    print(f"[PYTHON EVAL] Could not parse {dataset_name} results.")
                    print(f"  stdout: {stdout[:300]}")

        for dataset_name, pass1 in eval_results_code.items():
            result_data[f"test_{dataset_name}_pass1"] = pass1

        if not args.no_wandb:
            import wandb
            if wandb.run is not None:
                for dataset_name, pass1 in eval_results_code.items():
                    wandb.run.summary[f"eval/final_test_{dataset_name}_pass1"] = pass1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Python Code Generation Training")
    ...
    # evalplus evaluation (HumanEval + MBPP)
    parser.add_argument("--eval_test_acc", action="store_true",
                        help="Run evalplus pass@1 evaluation after training (HumanEval + MBPP)")
    parser.add_argument("--max_test_samples", type=int, default=0,
                        help="Max test samples for evalplus (0=all; humaneval=164, mbpp=399)")
    parser.add_argument("--test_max_new_tokens", type=int, default=512,
                        help="Max new tokens to generate for code evaluation")
    parser.add_argument("--save_test_responses", action="store_true",
                        help="Save generated code completions to JSONL files in results/")
