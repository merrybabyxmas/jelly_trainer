#!/usr/bin/env python
"""
Conversation Instruction-Following Comparison Experiment
=========================================================
JELLY vs LoRA, PiSSA, DoRA, BitFit on Conversation dataset
...
LLM_CSV_COLUMNS = ["method", "params(%)", "eval_loss", "eval_perplexity", "last_train_loss"]


class ConveisionComparisonRunner(BaseExperimentRunner):
    """Conversation instruction-following comparison experiment (병렬 GPU 실행 지원)"""
    ...
    def __init__(self, seeds=None, gpus="0", per_gpu_tasks=1, test_mode=False,
                 ...
                 data_path="fxmeng/pissa-dataset", sub_task="conversation",
                 eval_mt_bench=False, max_mt_bench_samples=0, mt_bench_max_new_tokens=512):
        super().__init__(
            experiment_name="conveision_comparison",
            seeds=seeds,
            gpus=gpus,
            ...
            use_wandb=use_wandb,
            wandb_project=wandb_project or "[JELLY]LLM-Conveision",
        ...
        self._eval_mt_bench = eval_mt_bench
        self._max_mt_bench_samples = max_mt_bench_samples
        self._mt_bench_max_new_tokens = mt_bench_max_new_tokens
    ...
    def build_llm_training_args(self, method: str):
        """Build args for train_conveision.py"""
        ...
        if self._eval_mt_bench:
            args.append("--eval_mt_bench")
            args.extend(["--max_mt_bench_samples", str(self._max_mt_bench_samples)])
            args.extend(["--mt_bench_max_new_tokens", str(self._mt_bench_max_new_tokens)])
    ...
    def run_single_experiment(self, method: str, seed: int, gpu_id: str = None) -> dict:
        ...
        if result_file.exists():
            with open(result_file, "r") as f:
                ...
                self.update_progress(f"{job_name} = eval_loss:{result['eval_loss']:.4f}")
        return result
    ...
    def run_all_experiments(self):
        ...
        self.log(f"")
        self.log(f"{'='*60}")
        self.log(f" Conveision Comparison 완료!")
        self.log(f" 결과: {self.csv_path}")
        self.log(f"{'='*60}")


def main():
    parser = argparse.ArgumentParser(description="Conveision Comparison Experiment (병렬 GPU 지원)")
    ...
    parser.add_argument("--max_mt_bench_samples", type=int, default=0,
                        help="Max MT-Bench questions (0=all 80)")
    ...
    runner = ConveisionComparisonRunner(
        seeds=seeds,
        gpus=args.gpus,
        ...
        eval_mt_bench=args.eval_mt_bench,
        max_mt_bench_samples=args.max_mt_bench_samples,
